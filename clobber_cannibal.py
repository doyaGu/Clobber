...
def possibleDestination(board, n, p, i, j, k, l):
    if isInBoard(n, p, k, l) and board[l][k] != 0:
        return (i == k and abs(j - l) == 1) or (j == l and abs(i - k) == 1)
    return False


def selectDestination(board, n, p, i, j):
    l = getInput('Select a destination, row : ')
    k = getInput('Select a destination, column : ')
    while not possibleDestination(board, n, p, i, j, k, l):
        l = getInput('Select a destination, row : ')
        k = getInput('Select a destination, column : ')
    return k, l
...
def clobber(n, p):
    ...
    while True:
        ...
        k, l = selectDestination(board, n, p, i, j)
        board[j][i] = 0
        board[l][k] = player
        print('\n')
